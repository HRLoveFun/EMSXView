...
- **典型用户任务**：每日定时抓取 fill 数据并跑完整管道；回填历史 BDIB 数据；按 broker/算法维度运行归因分析；查询 TCA 报告和评分卡
- **主要入口**：
...
- **主要代码路径**：`MarketView/backend/api/routers/marketview.py`、`ExecutionView/frontend/src/modules/marketview/`
- **关键依赖**：platform_data（MarketReferenceDataAdapter）、CostView 管道（bdib_daily_summary 数据源）
- **实现状态**：Shell anchor + 后端路由已就位，独立数据和业务逻辑尚待建设。


## 子系统 5: Database
- **业务职责**：存储、更新、处理交易记录和市场历史数据
- **典型用户任务**：
- **主要入口**：[从入口清单挑,要是不在清单上，在项目中找，找到了补到入口清单]
- **主要代码路径**：data
- **关键依赖**：
