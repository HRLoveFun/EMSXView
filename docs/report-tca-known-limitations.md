...
| 4 | 受影响范围 | 抽样 3,000 个 (ticker, 日期) 组合，**首 bar 异常率 3.3%**（≈ chunk 边界率，与「依赖性划分」一致） |
| 5 | **铁证（rowid 相邻归因）** | 每条极端行的 `rowid−1` 均为**另一 ticker 的 16:00 收盘 bar**：`NSC 09:30` ← `FTV 16:00`、`LIN 09:30` ← `BALL 16:00`、`ZM 09:30` ← `CBRE 16:00`、`DOV 09:30` ← `ZM 16:00` … |

第 5 条同时说明数据的物理排列是「按 ticker 分组、组内按时间有序」，而 `shift(1)` 在
**全局**范围做 —— 因此每个 ticker 组的首行取到了**上一组的末行**，与上游描述完全一致。

**一处未能对上、如实记录**：`ln(fill_px_本行 / fill_px_前一行)` 与列值**不吻合**
（10 条抽查 0 条吻合，如 `NSC` 列值 −5.718 vs 重算 +1.608）。原因判断：
`log_chg_pct_10s` 的原计算基准价格列在这些行**为 NULL**（`close` 非空率 64.4%、
`vwap` 34.5%，仅 `fill_px` 100%），故无法用现存列反推其原分母。
**这不影响「分母跨 ticker」的判定**（第 5 条已独立成立），但意味着**该缺陷的精确数值
无法从现存数据还原** —— 这也正是「依赖 chunk 划分、不可复现」的直接体现。
