...
| T22 | `raw_bdib` 数据完整性问题（两类）：① **市场级缺口** —— `20260420` 仅 9 个市场 / 431 ticker、`20260421` 仅 7 个 / 371 ticker（正常日 23 个 / ~2,160），保留市场的 ticker 数与正常日**逐一完全相同** → 上游归因为**按市场分批采集，该两日只完成前 8~9 个市场**（形态与 `backfill_bdib_by_market.py` 一致）；② **桶对齐异常** —— `20260408` 的 `:00` 秒桶比其它秒位**高 63%**（正常日各秒位均匀），该日 fill 行仅 **18.3%** 能在 `raw_bdib` 找到同 10 秒桶（正常日 88.1%） | 本侧实测第二十五轮 ⑤；上游诊断第二十六轮 二/三 | ⏳ **已向上游请求两项后续** | **本侧已决策（第二十六轮）**：① 请求上游核对 `backfill_bdib_by_market.py` 的市场清单/顺序（把「形态吻合」做成实锤）；② **请求上游为 `raw_bdib` 增加市场级完整性告警**（治本 —— 这类缺口的表现为**低估而非报错**，是最危险的失败模式，写入侧告警可在产生时暴露）。§ **本侧影响**：`20260420/0421` 的 `log_chg_pct_10s` 只覆盖部分 ticker、`20260408` 的 10 秒桶对齐基准与其它日不同（影响不止 `log` 一列，任何依赖 10 秒桶对齐的分析在该日都需注意）→ 作这些日期的分析时会**低估**，需在文档标注 |
