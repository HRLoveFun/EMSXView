...
| T19 | **4-22 批次 36,367 行权威覆盖**（用 Bloomberg 原始值替换现有近似值）—— 上游已就绪，**等待本侧授权** | 上游通知 2026-09-22（第三轮 §3.2 / §六） | ⏳ | 上游脚本 `verify_volatility_against_bloomberg.py --apply`（覆盖前自动备份、可回滚）；本侧建议**授权**（现状约 56% 行偏高 1.25~5 倍，该区间的分层结论不可信）。授权后上游回报覆盖行数与 `ratio` 统计 |
