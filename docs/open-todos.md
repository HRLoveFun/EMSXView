...
| T20 | 上游 `compute_derived_fields` 的 `log_chg_pct_10s` 用**未分组** `shift(1)` —— 多 ticker 合并输入下每个 ticker 首 bar 与上一个 ticker 末 bar 相除，结果**依赖 chunk 划分（不可复现）** | 上游通知 2026-09-22（第三轮 §四-④） | ⏳ | 上游已登记、**未改动**，待其设计确认；**本侧若启用 `fill_bdib` 的 `cum_interval_volatility` / `standard_cum_interval_volatility` 做分析，须先等此修复**（否则结果不可复现） |
| T21 | 前端「评估失败」收尾验证：backend（端口 **3000**）实测已为新代码（`evaluation` 位为真、`/api/tca/evaluation/report` 返回 **200**），故嫌疑在**前端旧 bundle**（仍调用 027 已移除的 `/compare`） | 本侧实测 2026-09-22（`docs/report-tca-known-limitations.md` 第二十二轮 ⑤） | ⏳ | 待办：重启前端 dev server / 硬刷新浏览器 → DevTools Network 确认请求路径为 `/api/tca/evaluation/report`（而非 `/compare`）；若仍失败，回传 `readError` 文本。**注**：该问题先后被误判三次（backend 未重启 / 端口不匹配 / backend 旧代码），均与实测不符 |
