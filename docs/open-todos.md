...
| T20 | 上游 `compute_derived_fields` 的 `log_chg_pct_10s` 用**未分组** `shift(1)` —— 多 ticker 合并输入下每个 ticker 首 bar 与上一个 ticker 末 bar 相除，结果**依赖 chunk 划分（不可复现）** | 上游通知 2026-09-22（第三轮 §四-④）；本侧实测第二十三轮；上游修复 `b153808` 与答复第二十四轮；**回填执行与复验第二十五轮** | 🟡 **已回填并复验通过；待确认 2025 年 5 天处置** | **代码已修复**（`b153808`：按 `(equ_ticker, order_as_of_date)` 分组 + `mkt_timestamp` 稳定排序 + 读取层显式 `ORDER BY` + 7 例单测）。**回填已执行**（上游脚本 `1f9983a`，82 天 / 更新 2,195,505 行）。§ **本侧独立复验五项全部对上**：`\|x\|>1` 690、`\|x\|>0.1` 1,078、`cum` 最大 0.025778、行数 6,680,277、`max\|log\|` 7.35930；**2026 年及以后 `\|x\|>0.1` = 0**；首 bar 契约**0 违反**（经 `raw_bdib` 对照判定）。§ **残留 690 行 100% 在 5 个 2025 日期**（`20251121/1124/1128/1205/1212`）—— 这 5 天（实为 **20 天**）在 `raw_bdib` 中**无 bars**，物理不可重算。精确范围：`log` 有值仅那 5 天的 **388,681 行**，`cum` **20 天全 NULL（无需处置）**。**本侧建议 A（置 NULL）**：该值无法验证且与 2026 段同源含污染（可检出仅 690 行，其余不可检出、无法区分）。**待用户确认后回复上游**。§ **新发现（登记 T22）**：`raw_bdib` 在 `20260420/0421` 仅有 431/371 个 ticker（正常日 ~2,160）→ 上游回填据此置 NULL 属**正确行为**（算不出即 NULL，不保留不可信旧值）；`20260408` 规模正常但非空由 24,710 降至 1,523，**原因待上游说明**。§ `journal_mode` 实测为 **`wal`**（`-wal` 0 字节、只读正常、本侧 79 测试通过 → **当前无影响**）；本侧无 BEFORE 快照，**不作归因、不要求改动**，仅登记「复制数据库须用 `VACUUM INTO` 或确认 checkpoint」。§ 本侧仍**不启用** `cum_interval_volatility` / `standard_cum_interval_volatility`（后者分母为批次全表非零均值，跨批次不可比且会把异常压平） |
| T22 | `raw_bdib` 存在 **ticker 级缺口**：`20260420` 仅 431 个 ticker、`20260421` 仅 371 个，而相邻正常日为 2,157~2,172（约 1/5） | 本侧实测 2026-09-22（`docs/report-tca-known-limitations.md` 第二十五轮 ⑤） | ⏳ | 该缺口**不是整天缺失**（否则重算会全 NULL），而是**部分 ticker 缺失**，故**无法从 `fill_bdib` 侧察觉**（只能看到结果 NULL）。抽样证据：`NVDA US Equity` 在 `20260421` 的 `fill_bdib` 有 638 个时点、`raw_bdib` **0 个**。**影响**：这些日期的 `log_chg_pct_10s` / `cum_interval_volatility` 只覆盖部分 ticker，作全市场分析会**低估**（而非报错）。**待上游核实**：是采集缺口、清洗过滤，还是 A8 退役后的清理残留 |
