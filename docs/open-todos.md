...
| T20 | 上游 `compute_derived_fields` 的 `log_chg_pct_10s` 用**未分组** `shift(1)` —— 多 ticker 合并输入下每个 ticker 首 bar 与上一个 ticker 末 bar 相除，结果**依赖 chunk 划分（不可复现）** | 上游通知 2026-09-22（第三轮 §四-④）；本侧实测见 `docs/report-tca-known-limitations.md` 第二十三轮 | 🔄 **本侧已启动排期请求（2026-09-22）** | **本侧已独立实测证实**（`fill_bdib` 只读）：① `log_chg_pct_10s` 值域 **−9.70 ~ +7.41**（10 秒对数收益不可能）；② `\|x\|>0.1` 共 **11,325 行**、`\|x\|>5` 共 307 行；③ 极端行**全部落在 09:30 开盘首 bar**，抽样 (ticker, 日期) 组合首 bar 异常率 **3.3%**（≈ chunk 边界率）；④ **铁证**：每条极端行的 `rowid-1` 均为**另一 ticker 的 16:00 收盘 bar**（如 `NSC 09:30` ← `FTV 16:00`、`LIN 09:30` ← `BALL 16:00`），即数据按 ticker 分组排列而 `shift(1)` 在全局范围做；⑤ 下游后果可直接观测：`cum_interval_volatility` 最大 **37.77**（正常日内 < 5）。**本侧在修复前不启用 `cum_interval_volatility` / `standard_cum_interval_volatility`**（后者分母为批次内全局均值，跨批次不可比） |
