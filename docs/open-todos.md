...
| T20 | 上游 `compute_derived_fields` 的 `log_chg_pct_10s` 用**未分组** `shift(1)` —— 多 ticker 合并输入下每个 ticker 首 bar 与上一个 ticker 末 bar 相除，结果**依赖 chunk 划分（不可复现）** | 上游通知 2026-09-22（第三轮 §四-④）；本侧实测第二十三轮；**上游修复 `b153808` 与答复见第二十四轮** | 🔄 **代码已修复；存量回填待本侧决定** | **上游已修复**（`b153808`：按 `(equ_ticker, order_as_of_date)` 分组 + 组内按 `mkt_timestamp` 稳定排序取前值 + 读取层显式 `ORDER BY` + 7 例单测；**本侧独立复跑 7 passed**）。§ 根因澄清：非 schema 问题，是**读取层与计算层的契约缺陷**（正确性依赖 SQL 返回顺序 / `pd.concat` 拼接顺序 / chunk 划分三件不受控的事），`ALTER TABLE` 解决不了。§ 全仓扫描确认**仅此一处**未分组。§ **存量未回填**：194 天中 **73 天**含可检出污染（`\|x\|>0.1` 共 11,325 行），**上界 113,685 行**（= (ticker, 日期) 组合数，本侧实测）；成本 **0.5~3 小时**、纯本地、**无需 Bloomberg**、可分批中断续跑。§ **待决定**：A 等启用前再做（上游建议）/ B 现在全量 194 天 / C 只重算 73 天（上游不建议，会留下「哪些日期可信」无法自证）。§ 本侧在回填前**不启用** `cum_interval_volatility` / `standard_cum_interval_volatility`（后者分母为批次全表非零均值，**跨批次不可比**，且会把异常压平而不具备检出能力 —— 上游已采纳此提醒） |
