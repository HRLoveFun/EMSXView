...
> `specs/009~026` 已全部归档，`specs/` 当前为空（该目录只承载在途计划），本清单「来源」列与活文档中的历史引用一律指向归档路径。
...
| T15 | 归档 spec 的未收尾事项（026）：**可选上游物化需求** —— ① 请上游将 `adv_20d` / `daily_volatility` 物化到 `tca_route_summary` 列（触发条件：跨库读取覆盖率不足或报告耗时不可接受）；② `fill_bdib.mkt_timestamp` 与 `raw_bdib.mkt_timestamp` 格式口径统一（触发条件：实测确认两表异格式且判定为上游定义不一致） | `docs/archive/2026-09-21/026-costview-algo-eval/plan.md` §6（U-1 / U-2） | ⏳ | 2026-09-21 随 spec 归档由计划 §6 转记于此；两项均为**可选性能优化型、非正确性前置**（阶段二已实测本侧可经 `bdib_daily_summary` 自给，见 plan §4.1.1） |
