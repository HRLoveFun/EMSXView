...
- **Next Assessment Due**: 2026-05-07
...
- **What to watch in backend logs** (`logger.info`):
  - `batch-route batch total=N concurrency=K wall_ms=W succeeded=… blocked=… failed=…` (per request)
  - `batch-route item key=… status=… rtt_ms=…` (per item)
...
- **Tuning criterion**: if a 50-order batch's `wall_ms` is approximately
  `50 × median(rtt_ms)`, EMSX is the bottleneck — leave concurrency at 5.
  If `wall_ms < 0.5 × (N × median(rtt_ms))`, consider lowering to 3 to
