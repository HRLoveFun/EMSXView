...
4. **跨模块数据访问通过共享适配层** — 符合 .github/agent.md 永久性约束
...
| 指标 | 当前值 | 目标值 |
|---|---|---|
| `pipeline.py` 中旧 DB 类实例化 | 0（✅ 迭代1完成） | 0 |
| `CostView/src/` 中裸 `sqlite3.connect()` | 0（✅ 迭代3完成，仅 `db/connection.py` 内部保留） | 0 |
| `platform_data/adapters.py` 对 CostView 深层导入 | 1 处 | 0 |
| `tca_query_service.py` 中裸 SQL 连接 | 0（✅ 迭代2完成） | 0 |
| `execution_history_service.py` 中裸 SQL 连接 | 0（✅ 迭代2完成） | 0 |
| `daily_metrics_calculator.py` 中裸 SQL 连接 | 0（✅ 迭代2完成） | 0 |
| `MigrationManager.ensure_current()` 对旧 DB 类依赖 | 0（✅ 迭代3完成） | 0 |
...
### 迭代 2：tca_query_service 迁移到 ConnectionManager ✅ 已完成
...
| # | 步骤 | 变更文件 | 风险 | 测试策略 |
|---|---|---|---|---|
| 2.1 | `ConnectionManager` 增强 `row_factory` 和 `path_overrides` 参数 | `db/connection.py` | 低 | 连接测试 | ✅ |
| 2.2 | `TcaQueryService.__init__` 接受 `ConnectionManager` 参数（保留 `db_path` 向后兼容，内部转换 `path_overrides`） | `tca_query_service.py` | 低 | 构造测试 | ✅ |
| 2.3 | 4 个 `_xxx_conn()` 工厂方法改用 `ConnectionManager.get_connection()` | `tca_query_service.py` | 中 | 连接测试 | ✅ |
| 2.4 | `_compute_route_metrics_from_raw_bdib` 类型注解更新 | `tca_query_service.py` | 低 | TCA 报告回归 | ✅ |
| 2.5 | `execution_history_service.py` 注入 `ConnectionManager` | `execution_history_service.py` | 中 | 历史查询回归 | ✅ |
| 2.6 | `daily_metrics_calculator.py` 注入 `ConnectionManager` | `daily_metrics_calculator.py` | 中 | Stage 7 回归 | ✅ |

#### 迭代 2 验收标准

- [x] `tca_query_service.py` 中零 `sqlite3.connect()` 调用
- [x] `execution_history_service.py` 中零 `sqlite3.connect()` 调用
- [x] `daily_metrics_calculator.py` 中零 `sqlite3.connect()` 调用
- [x] TCA 测试套件 42/42 通过
- [x] Pipeline 守卫测试 15/17 通过（2 个预先存在的失败与本次变更无关）

---

### 迭代 3：旧 DB 类内部迁移 + 辅助文件清理 ✅ 已完成
...
| 3.6 | `validate_raw_fills.py`、`query_cli.py` 改用 ConnectionManager | `validate_raw_fills.py`, `query_cli.py` | 低 | 脚本功能验证 | ✅ |
| 3.7 | regime 模块全面迁移至 ConnectionManager + 修复 run_journal 显式 commit | `regime/schema.py`, `regime/migrations/apply.py`, `regime/fill_regime_tagger.py`, `regime/run_journal.py` | 中 | regime pipeline 回归 | ✅ |
| 3.8 | `attribution/repositories.py` SqliteFillRepository + SqliteBarDataRepository 注入 ConnectionManager | `attribution/repositories.py` | 中 | attribution 测试 | ✅ |
| 3.9 | `MigrationManager._ensure_inline_schema()` 使用 `db/schema/inline_ddl.py` 独立 DDL 函数，不再依赖旧 DB 类 | `db/schema/inline_ddl.py` (新增), `db/schema/migrations/manager.py` | 中 | 迁移测试 | ✅ |

#### 迭代 3 验收标准

- [x] `CostView/src/db/` 之外零 `sqlite3.connect()` 调用
- [x] 所有旧 DB 类内部使用 `ConnectionManager`
- [x] `MigrationManager` 不再依赖旧 DB 类进行 schema 初始化
- [x] Pipeline 完整运行无回归
- [x] 所有现有测试通过（81/81）
...
| 4.6 | 更新 `docs/spec/project-structure.md`、`docs/spec/data-domain.md`、`.github/agent.md` 相关描述 | 文档 | 低 | 人工审查 |
...
| `CostView/src/db/schema/inline_ddl.py` | 3 | 5 个数据库的 CREATE TABLE IF NOT EXISTS 独立 DDL 函数 |
...
|---|---|
| **触发条件** | APPLY 完成 |
| **Agent 行为** | (1) 追加条目到 `.github/knowledge/iteration-log.md`；(2) 如解决错误，检查 `.github/knowledge/error-patterns.md` 是否需要录入新模式（出现 2+ 次才录入）；(3) 如涉及架构变更，更新 `.github/knowledge/architecture-decisions.md`；(4) 如涉及用户需求，更新 `.github/knowledge/user-needs.md`；(5) 更新本 plan.md 中当前迭代的状态为 `完成`，标记下一迭代为 `待启动`；(6) 检查是否需要更新 `docs/spec/memory.md`、`docs/handoff.md` |
| **人工职责** | 审查文档完整性 |
...
```
当前: 迭代 3 已完成 (BUILD + QA)
迭代 1 状态: ✅ 完成 — Commit 1c20e9b
迭代 2 状态: ✅ 完成
迭代 3 状态: ✅ 完成 — 81/81 测试通过，db/ 之外零裸 sqlite3.connect()
迭代 4 状态: 待启动
下一步: 进入迭代 4 的 PLAN 阶段
```
