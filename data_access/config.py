r"""EMSXView 只读数据访问层 — 配置常量（010-extract-pipeline D3）。

裁剪自独立仓库 ``EMSXDataPipeline/DataPipeline/config.py``——库/表/路径
常量是两仓共享的**数据契约**，由契约测试锁定一致性。数据更新维护（写路径）
在独立仓库进行；本仓库仅为只读消费者。

Data directory resolution (010-extract-pipeline):

1. ``EMSXVIEW_DATA_DIR`` environment variable (explicit override, wins).
2. Default: ``D:\db`` — 数据独立于任何代码树，由独立仓库 EMSXDataPipeline
   维护，本仓库只读消费。
"""
...
class Config:
    ...
    # ── 数据目录解析（010-extract-pipeline）───────────────────────────────
    # 优先级：EMSXVIEW_DATA_DIR 环境变量 > 默认值 D:\db（独立数据盘，与代码树解耦）。
    DEFAULT_DATA_DIR: Path = Path(r"D:\db")
