...
# 或跨平台命令:
cd emsx-backend/backend && python -m uvicorn main:app --port 3000
...
| `npm run build` | 前端构建 |
| `npm run lint` | 前端代码检查 |
| `python -m py_compile emsx-backend/backend/main.py` | Python 语法检查 |
...
1. 在 GUIDE 中查找精确字段名
2. 在 `emsx-backend/backend/main.py` 的 `SUBSCRIPTION_FIELDS` 列表中添加
3. 在 `app/src/types/index.ts` 的 `OrderField` 类型中添加
4. 如需显示，更新对应 UI 组件
5. 测试: 访问 `/api/orders` 验证字段存在

### 修改订单表格 UI

1. 编辑 `app/src/sections/OrderTable.tsx`
...
1. 检查日志: `emsx-backend/backend/logs/emsx_api.log` (首次运行后端时自动创建目录)
...
首次运行后端时自动创建: `emsx-backend/backend/logs/`
