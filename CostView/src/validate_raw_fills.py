...
Usage modes:
    1. Standalone CLI: python -m src.validate_raw_fills [--date YYYYMMDD] [--output DIR]
...
Examples:
  ...
  python -m src.validate_raw_fills

  # Validate specific date
  python -m src.validate_raw_fills --date 20260327

  # Validate all dates with reports saved
  python -m src.validate_raw_fills --all-dates --output ./reports

  # Custom tolerance
  python -m src.validate_raw_fills --tolerance 0.001
        """,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Directory to save anomaly reports (.xlsx). Default: data/",
    )
    parser.add_argument(
        "--quiet", action="store_true",
        help="Only output machine-readable JSON summary",
    )
    ...
    if args.all_dates:
        ...
        if args.output and not result.anomalies_df.empty:
