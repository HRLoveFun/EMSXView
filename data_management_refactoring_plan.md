...
两个文件通过步骤编号（A1-A8, B1-B4, C1-C2, D1-D3）和参数名对齐。控制中心的每一步链接回本文件对应节号。
...
| **Phase D: 监控** |||||
| D1 | `scripts/health_check.py` — DB体积监控 (单文件>10 GB, 总热数据>50 GB) | ✅ | — | — | 独立 |
| D2 | `scripts/health_check.py` — TCA查询延迟监控 (P95 <3秒) | ✅ | — | — | 独立 |
| D3 | `scripts/health_check.py` — WAL文件大小监控 (>500 MB告警) | ✅ | — | — | 独立 |
...
Week 6: C1-C2, D1-D3  (归档调度 + 监控)
  └─ 门禁: 归档dry-run无异常, 健康检查全部通过 (DB体积/查询延迟/WAL)
```
...
| 触发条件 | .BAK处理 | 处置动作 |
|---------|---------|---------|
| 14天观察期全部通过 + 无阻断 | → 改为只读, 保留30天后自动清理 | 正常关闭 |
| 观察期内任何一天检查失败 | → 重置计数, 从失败日重新算14天 | 延长观察期, 不解冻 |
| 硬性阻断条件触发 | → 永久保留 | **先诊断修复, 仅在必要时回退** |
| 人工标注"疑似异常" | → 永久保留 | 人工调查 |

**硬性阻断处置流程**:
1. freeze: .BAK→永久保留, observation→final_status="blocked"
2. diagnose: 用 verify_bdib_engines.py 定位漂移根因 (收缩后的DB在线)
3. judge: 评估业务影响 (TCA偏差是否影响交易决策?)
4. fix: 修复代码/数据, 重新验证 (修复完直接在收缩后DB上验证)
5. rollback: 仅当步骤3判定不可容忍 AND 步骤4无法快速修复时, 从.BAK恢复
---
