"""MarketView Pydantic schemas — request/response models and serialization helpers.

Extracted from routers/marketview.py per P3 SRP refactoring. The router now
focuses on endpoint logic only, while models and serializers live here.
"""
...
# ── Market snapshot response models ───────────────────────────────────────
...
# ── Intraday feature response models ─────────────────────────────────────
...
# ── Handoff (WBS-08) request/response models ──────────────────────────────


class HandoffMetadataResponse(BaseModel):
    """Shared handoff metadata model (WBS-08 contracts 1-3).

    P2-B5: Made public (no leading underscore) so both routers/marketview.py
    and routers/orders.py can import without duplicating the model.
    Previously: _HandoffMetadataResponse (private, only marketview.py).
    """
...
# ── Execution-side handoff models (contract 2: Execution → Cost) ──────────
# P2-B5: Moved from routers/orders.py to eliminate duplication with the
# canonical schemas layer. Both routers/marketview.py and routers/orders.py
# now import their handoff models from this file.
...
def serialize_snapshot(snapshot: MarketSnapshot) -> MarketSnapshotPayload:
    """Convert MarketSnapshot domain object to Pydantic response model."""
...
def serialize_intraday_bucket(bucket: IntradayFeatureBucket) -> IntradayFeatureBucketResponse:
    return IntradayFeatureBucketResponse(**bucket.__dict__)


def serialize_intraday_ticker(features: IntradayTickerFeatures) -> IntradayTickerFeaturesResponse:
    data = {k: v for k, v in features.__dict__.items() if k != "buckets"}
    return IntradayTickerFeaturesResponse(
        **data,
        buckets=[serialize_intraday_bucket(bucket) for bucket in features.buckets],
    )


def serialize_intraday_snapshot(
    ...
    return IntradayFeatureSnapshotPayload(
        ...
        tickers=[serialize_intraday_ticker(ticker) for ticker in snapshot.tickers],
    )


def serialize_handoff_metadata(metadata) -> HandoffMetadataResponse:
...
def serialize_market_handoff(handoff) -> MarketToExecutionHandoffPayload:
    return MarketToExecutionHandoffPayload(
        metadata=serialize_handoff_metadata(handoff.metadata),
